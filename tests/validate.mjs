...
assert.match(html,/wss:\/\/\*\.supabase\.co/);
assert.match(html,/https:\/\/esm\.sh/);
assert.doesNotMatch(html,/firebase/);
assert.doesNotMatch(html,/<script[^>]+googletagmanager/);
const runtime=readFileSync(new URL('../supabase-runtime-config.js',import.meta.url),'utf8');
assert.match(runtime,/glknsljbqofbkchclfba\.supabase\.co/);
assert.match(runtime,/sb_publishable_/);
assert.doesNotMatch(runtime,/sb_secret_/);
const migration=readFileSync(new URL('../supabase/migrations/001_team_quest.sql',import.meta.url),'utf8');
for(const table of ['team_quest_config','team_sessions','team_players','team_answers','team_scorecards'])assert.match(migration,new RegExp(`create table if not exists public\\.${table}`));
assert.match(migration,/enable row level security/);
assert.match(migration,/cleanup_expired_team_quest/);
